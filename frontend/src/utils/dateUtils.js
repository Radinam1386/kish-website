...
export function getExamTimeStatus(exam, userId) {
  if (!exam) return { status: "unknown", canStart: false, canResume: false };

  const now = new Date();

  let isoDate = exam.date;
  if (typeof isoDate === "string") {
    const clean = toEnglishDigits(isoDate.trim());
    if (/^14\d{2}[/-]/.test(clean)) {
      isoDate = parseJalaliStringToGregorian(clean);
    }
  }

  const startTimeStr = (exam.start_time || "00:00").slice(0, 5);
  const endTimeStr = (exam.end_time || "23:59").slice(0, 5);
  ...
  if (isoDate) {
    ...
    if (parts.length === 3) {
      ...
      startWindow = new Date(year, month - 1, day, startH || 0, startM || 0, 0, 0);
      endWindow = new Date(year, month - 1, day, endH || 23, endM || 59, 59, 999);
    }
  }

  const actualUserId = userId || "guest";
  const storageKey = `kish_exam_start_time_${exam.id}_${actualUserId}`;
  const localStartTime = localStorage.getItem(storageKey);

  const durMinutes = Number(exam.duration_minutes) || 45;
  const durSeconds = durMinutes * 60;

  if (localStartTime) {
    const elapsed = Math.floor((Date.now() - Number(localStartTime)) / 1000);
  ...
  if (startWindow && now < startWindow) {
    return {
      status: "upcoming",
      statusLabel: "هنوز آغاز نشده",
      windowLabel: `شروع از ساعت ${toPersianDigits(startTimeStr)}`,
      canStart: false,
      canResume: false,
      startTimeStr,
      endTimeStr,
  ...
  if (endWindow && now > endWindow) {
    return {
      ...
      windowLabel: `مهلت ورود تا ${toPersianDigits(endTimeStr)} بود`,
      canStart: false,
      canResume: false,
      startTimeStr,
      endTimeStr,
  ...
  return {
    ...
    windowLabel: `مهلت ورود: ${toPersianDigits(startTimeStr)} تا ${toPersianDigits(endTimeStr)}`,
    canStart: true,
    canResume: false,
    startTimeStr,
    endTimeStr,
